        ...
        <div className="card flex flex-col gap-2 col-span-full w-full">
            ...
            <div className="flex flex-wrap gap-2">
                {servers.servers.slice(0, 4).map((server) => (
                    <div key={server.bm_id} className="flex gap-0 p-1 flex-col bg-[#1C221B] w-full md:max-w-[calc(50%-0.4rem)] xl:max-w-[calc(25%-0.4rem)]">
                        {
                            server.header_url && (
                                <Tooltip trigger={
                                ...
                                >
                                    <div className="space-y-4 max-w-md">
                                        <div className="text-primary font-bold text-roboto">{server.name}</div>
                                        ...
                                        <div className="grid grid-cols-1 gap-2 text-sm text-roboto">
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span className={`pl-2 px-1.5 py-0.5 rounded text-xs ${server.status === 'online' ? 'bg-green-700' : 'bg-red-700'}`}>
                                            ...
                                            {server.time_played && (
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <span>Played: {formatTimePlayed(server.time_played)}</span>
                                                </div>
                                            )}
                                            ...
                                            {server.last_seen && (
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <span>Last visit: {formatDistanceToNow(parseISO(server.last_seen), {
                                                        addSuffix: true
                                                    }).replace(/^about /, '')}</span>
                                                </div>
                                            )}
                                            ...
                                            {server.first_seen && (
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <span>First visit: {formatDistanceToNow(parseISO(server.first_seen), {
                                                        addSuffix: true
                                                    }).replace(/^about /, '')}</span>
                                                </div>
                                            )}
                                        ...
                                        <div className="flex flex-wrap gap-2 text-xs">
                                            {server.is_official && (
                                                <span className="px-2 py-1 bg-blue-700 rounded">Official</span>
                                            )}
                                            {server.rust_type && (
                                                <span className="px-2 py-1 bg-orange-700 rounded capitalize">{server.rust_type}</span>
                                            )}
                                            {server.gamemode && (
                                                <span className="px-2 py-1 bg-purple-700 rounded capitalize">{server.gamemode}</span>
                                            )}
                                        </div>

                                        {server.url && (
                                            <div className="text-sm text-roboto">
                                                <a href={server.url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">
                                                    {server.url}
                                                </a>
                                            </div>
                                        )}
                                        ...
                                        {server.description && (
                                            <div className="text-roboto border-t border-gray-700 pt-2 mt-2">
                                                <p className="text-sm whitespace-pre-line">{server.description}</p>
                                            </div>
                                        )}
                                    </div>
                                </Tooltip>
                        ...
                        </div>
                        {
                            server.last_seen && (
                                <div className="text-roboto text-secondary text-end pb-1 px-1">
                                    Last online {formatDistanceToNow(parseISO(server.last_seen), {
                                        addSuffix: true
                                    }).replace(/^about /, '')}
                                </div>
                            )
