...
export const Modal: FC<PropsWithChildren<{size?: 'small' | 'medium' | 'large'}>> = ({ children, size = 'small' }) => {
  return (
    <DialogPortal>
      ...
      <DialogContent className={`fixed left-1/2 top-1/2 max-h-[85vh] w-[90vw] -translate-x-1/2 -translate-y-1/2 rounded-md bg-secondary p-[25px] shadow-md focus:outline-none data-[state=open]:animate-contentShow overflow-y-auto flex flex-col gap-4 ${size === 'small' ? 'max-w-[500px]' : size === 'medium' ? 'max-w-[800px]' : 'max-w-[1000px]'}`}>
        {children}
				<DialogClose asChild>
					<button
