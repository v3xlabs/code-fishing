...
export const PartyInviteCard: FC<{ partyId: string }> = ({ partyId }) => {
    ...
    return (
        <div className="card flex flex-col gap-2 max-w-full w-full text">
            <h2>Party Invite</h2>
            <p className="text-secondary">
                Share this link with your friends to invite them to the party.
            </p>
            <div className="flex justify-center gap-2 flex-col items-center">
                <QRCode value={`${window.location.origin}/${partyId}`} hidden={hidden} />
                <div className="text-secondary">
                    Party Code:{' '}
                    <span className="inline-flex font-bold bg-tertiary px-2 py-1">
                        {hidden ? '*'.repeat(partyId.length) : partyId}
                    </span>
                </div>
            </div>
            <div className="flex gap-2 w-full flex-wrap justify-between">
                <input
                    ...
                    className="input flex-1 sm:w-[240px]"
                    value={`${window.location.origin}/${partyId}`}
                    disabled
                />
                <button
                    className="button flex-1 inline-flex items-center justify-center gap-2"
                    onClick={() => handleCopy(partyId)}
                >
                    <LuClipboard />
                    Copy
                </button>
    ...
    return (
        <div className="bg-tertiary p-2 rounded-md">
            <div className="w-full h-full bg-secondary rounded-md">
                {hidden ? (
                    <div className="w-32 h-32 bg-tertiary rounded-md flex items-center justify-center gap-2">
