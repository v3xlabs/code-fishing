...
export const PartyMemberPreview: FC<PropsWithChildren<{ member: User }>> = ({ member, children }) => {
    ...
    const calculateDeathPercentages = () => {
        if (!stats?.deaths?.total) return { fall: 0, suicide: 0, selfInflicted: 0, other: 0 };
        ...
        const selfInflicted = parseInt(stats.deaths.self_inflicted.replace(/,/g, ''));
                ...
                <div className="w-full space-y-4">
                    ...
                    <div className="bg-tertiary p-6 flex items-center gap-4 border-b border-neutral-700">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-2">
                                <h3 className="text-xl font-semibold">{stats?.personaname || member.name || "Unknown Player"}</h3>
                                {
                                    !stats?.is_banned && (
                                        <Tooltip trigger={
                        ...
                        <div>
                            <a href={`https://steamcommunity.com/profiles/${member.user_id.replace("steam:", "")}`} target="_blank" rel="noopener noreferrer" className="button button-rust !px-4 !py-2 flex items-center gap-2">
                                <FaSteam size={16} />
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {stats?.kills && (
                                <div className="card no-padding">
                                    <h4 className="text-secondary uppercase text-xs font-semibold tracking-wider mb-3">Kills</h4>
                                    <div className="grid grid-cols-3 gap-3">
                                        <KillStatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/deermeat-cooked.webp" className="w-8 h-8" alt="Deer" />}
                                            value={stats.kills.deer || "0"}
                                            label="Deer"
                                        />
                                        <KillStatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/bearmeat-cooked.webp" className="w-8 h-8" alt="Bear" />}
                                            value={stats.kills.bears || "0"}
                                            label="Bears"
                                        />
                                        <KillStatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/meat-pork-cooked.webp" className="w-8 h-8" alt="Boar" />}
                                            value={stats.kills.boars || "0"}
                                            label="Boars"
                                        />
                                        <KillStatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/wolfmeat-cooked.webp" className="w-8 h-8" alt="Wolf" />}
                                            value={stats.kills.wolves || "0"}
                                            label="Wolves"
                                        />
                                        <KillStatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/chicken-cooked.webp" className="w-8 h-8" alt="Chicken" />}
                            ...
                            {stats?.deaths && (
                                <div className="card no-padding">
                                    <h4 className="text-secondary uppercase text-xs font-semibold tracking-wider mb-3">Deaths</h4>
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-tertiary rounded p-3">
                                            <div className="flex justify-between mb-1">
                                                <span className="text-xs text-secondary">Fall</span>
                                                <span className="text-xs text-secondary">{deathPercentages.fall}%</span>
                                            </div>
                                            <div className="w-full bg-primarybg rounded-full h-2 mb-1">
                                                <div className="bg-accent h-2 rounded-full" style={{ width: `${deathPercentages.fall}%` }}></div>
                                            </div>
                                            <div className="text-xs text-secondary text-center">({stats.deaths.fall})</div>
                                        </div>
                                        <div className="bg-tertiary rounded p-3">
                                            <div className="flex justify-between mb-1">
                                                <span className="text-xs text-secondary">Suicide</span>
                                                <span className="text-xs text-secondary">{deathPercentages.suicide}%</span>
                                            </div>
                                            <div className="w-full bg-primarybg rounded-full h-2 mb-1">
                                                <div className="bg-rust h-2 rounded-full" style={{ width: `${deathPercentages.suicide}%` }}></div>
                                            </div>
                                            <div className="text-xs text-secondary text-center">({stats.deaths.suicide})</div>
                                        </div>
                                        <div className="bg-tertiary rounded p-3">
                                            <div className="flex justify-between mb-1">
                                                <span className="text-xs text-secondary">Self Inflicted</span>
                                                <span className="text-xs text-secondary">{deathPercentages.selfInflicted}%</span>
                                            </div>
                                            <div className="w-full bg-primarybg rounded-full h-2 mb-1">
                                                <div className="bg-rust-active h-2 rounded-full" style={{ width: `${deathPercentages.selfInflicted}%` }}></div>
                                            </div>
                                            <div className="text-xs text-secondary text-center">({stats.deaths.self_inflicted})</div>
                                        </div>
                                        <div className="bg-tertiary rounded p-3">
                                            <div className="flex justify-between mb-1">
                                                <span className="text-xs text-secondary">Other</span>
                                                <span className="text-xs text-secondary">{deathPercentages.other}%</span>
                                            </div>
                                            <div className="w-full bg-primarybg rounded-full h-2 mb-1">
                                                <div className="bg-secondarybg h-2 rounded-full" style={{ width: `${deathPercentages.other}%` }}></div>
                                            </div>
                                            <div className="text-xs text-secondary text-center">({(parseInt(stats.deaths.total.replace(/,/g, '')) -
                                                parseInt(stats.deaths.fall.replace(/,/g, '')) -
                                                parseInt(stats.deaths.suicide.replace(/,/g, '')) -
                                                parseInt(stats.deaths.self_inflicted.replace(/,/g, '')))})</div>
                                        </div>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {stats?.melee && (
                                <div className="card no-padding">
                                    <h4 className="text-secondary uppercase text-xs font-semibold tracking-wider mb-3">Melee</h4>
                                    <div className="grid grid-cols-2 gap-3">
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/spear-wooden.webp" className="w-8 h-8" alt="Throws" />}
                                            label="Throws"
                                            value={stats.melee.throws || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/knife-bone.webp" className="w-8 h-8" alt="Strikes" />}
                                            label="Strikes"
                            ...
                            {stats?.bow_hits && (
                                <div className="card no-padding">
                                    <h4 className="text-secondary uppercase text-xs font-semibold tracking-wider mb-3">Bow Accuracy</h4>
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-tertiary rounded p-3">
                                            <div className="flex justify-between items-center">
                                                <span className="text-secondary text-sm">Overall</span>
                                                <span className="text-primary text-sm font-semibold">{stats.bow_hits.rate || "0%"}</span>
                                            </div>
                                            <div className="flex justify-between text-xs text-secondary mt-1">
                                                <span>Shots: {stats.bow_hits.shots_fired || "0"}</span>
                                            </div>
                                        </div>
                                        <div className="bg-tertiary rounded p-3">
                                            <div className="flex justify-between items-center">
                                                <span className="text-secondary text-sm">Players</span>
                                                <span className="text-primary text-sm font-semibold">{stats.bow_hits.players || "0"}</span>
                                            </div>
                                        </div>
                                        <div className="bg-tertiary rounded p-3">
                                            <div className="flex justify-between items-center">
                                                <span className="text-secondary text-sm">Buildings</span>
                                                <span className="text-primary text-sm font-semibold">{stats.bow_hits.buildings || "0"}</span>
                                            </div>
                                        </div>
                                        <div className="bg-tertiary rounded p-3">
                                            <div className="flex justify-between items-center">
                                                <span className="text-secondary text-sm">Animals</span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {stats?.other && (
                                <div className="card no-padding">
                                    <h4 className="text-secondary uppercase text-xs font-semibold tracking-wider mb-3">Other Activities</h4>
                                    <div className="grid grid-cols-2 gap-3">
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/research-table.webp" className="w-8 h-8" alt="BPs" />}
                                            label="BPs Learned"
                                            value={stats.other.bps_learned || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/modular-car.webp" className="w-8 h-8" alt="Cars" />}
                                            label="Cars Shredded"
                                            value={stats.other.cars_shredded || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/ammo-rocket-basic.webp" className="w-8 h-8" alt="Rockets" />}
                                            label="Rockets Fired"
                                            value={stats.other.rockets_fired || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/megaphone.webp" className="w-8 h-8" alt="Voice" />}
                                            label="Voice Chat"
                            ...
                            {stats?.exposure && (
                                <div className="card no-padding">
                                    <h4 className="text-secondary uppercase text-xs font-semibold tracking-wider mb-3">Environmental Exposure</h4>
                                    <div className="grid grid-cols-2 gap-3">
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/snowball.webp" className="w-8 h-8" alt="Cold" />}
                                            label="Cold Exposure"
                                            value={stats.exposure.cold || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/cactusflesh.webp" className="w-8 h-8" alt="Heat" />}
                                            label="Heat Exposure"
                                            value={stats.exposure.heat || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/hazmatsuit.webp" className="w-8 h-8" alt="Radiation" />}
                                            label="Radiation"
                                            value={stats.exposure.radiation || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/sofa.webp" className="w-8 h-8" alt="Comfort" />}
                                            label="Comfort Time"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {stats?.gathered && (
                                <div className="card no-padding">
                                    <h4 className="text-secondary uppercase text-xs font-semibold tracking-wider mb-3">Resources Gathered</h4>
                                    <div className="grid grid-cols-3 gap-3">
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/wood.webp" className="w-8 h-8" alt="Wood" />}
                                            label="Wood"
                                            value={stats.gathered.wood || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/stones.webp" className="w-8 h-8" alt="Stone" />}
                                            label="Stone"
                                            value={stats.gathered.stone || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/metal-ore.webp" className="w-8 h-8" alt="Metal" />}
                                            label="Metal Ore"
                                            value={stats.gathered.metal_ore || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/cloth.webp" className="w-8 h-8" alt="Cloth" />}
                                            label="Cloth"
                                            value={stats.gathered.cloth || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/leather.webp" className="w-8 h-8" alt="Leather" />}
                                            label="Leather"
                                            value={stats.gathered.leather || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/lowgradefuel.webp" className="w-8 h-8" alt="Low Grade" />}
                                            label="Low Grade"
                                            value={stats.gathered.low_grade_fuel || "0"}
                                        />
                                        <StatItem
                                            icon={<img src="https://cdn.rusthelp.com/images/256/scrap.webp" className="w-8 h-8" alt="Scrap" />}
                                            label="Scrap"
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                ...
                                {stats?.building_blocks && (
                                    <div className="card no-padding">
                                        <h4 className="text-secondary uppercase text-xs font-semibold tracking-wider mb-3">Building</h4>
                                        <div className="grid grid-cols-2 gap-3">
                                            <StatItem
                                                icon={<img src="https://cdn.rusthelp.com/images/256/building-planner.webp" className="w-8 h-8" alt="Placed" />}
                                                label="Blocks Placed"
                                                value={stats.building_blocks.placed || "0"}
                                            />
                                            <StatItem
                                                icon={<img src="https://cdn.rusthelp.com/images/256/hammer.webp" className="w-8 h-8" alt="Upgraded" />}
                                ...
                                {stats?.horse_distance_ridden && (
                                    <div className="card no-padding">
                                        <h4 className="text-secondary uppercase text-xs font-semibold tracking-wider mb-3">Travel</h4>
                                        <div className="grid grid-cols-2 gap-3">
                                            <StatItem
                                                icon={<img src="https://cdn.rusthelp.com/images/256/horse-armor-roadsign.webp" className="w-8 h-8" alt="Distance (mi)" />}
                                                label="Horse Distance (mi)"
                                                value={stats.horse_distance_ridden.miles || "0"}
                                            />
                                            <StatItem
                                                icon={<img src="https://cdn.rusthelp.com/images/256/horse-saddle-single.webp" className="w-8 h-8" alt="Distance (km)" />}
...
const StatItem: FC<{ icon: React.ReactNode; label: string; value: string | number }> = ({ icon, label, value }) => (
...
const KillStatItem: FC<{ icon: React.ReactNode; label: string; value: string | number }> = ({ icon, label, value }) => (
    <Tooltip trigger={
        ...
        <div className="text-xs text-secondary">{label} Killed: {value}</div>
    </Tooltip>
);

// Helper component for stat value display
const StatValue: FC<{ label: string; value: string | number }> = ({ label, value }) => (
